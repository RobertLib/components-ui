...
export default function PropsTable({ of, title }: PropsTableProps) {
  const type = docs.types[`${of}Props`] ?? docs.types[of];
  const defaults = docs.components[of]?.defaults ?? {};
    ...
    <div className="my-6">
      {title !== "" && (
        <h3 className="mb-2 font-mono text-sm font-semibold text-neutral-900 dark:text-neutral-100">
          {title ?? (docs.types[`${of}Props`] ? `${of}Props` : of)}
        </h3>
      )}
      <div className="overflow-x-auto rounded-lg border border-neutral-200 dark:border-neutral-800">
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead className="bg-neutral-50 text-xs tracking-wide text-neutral-500 uppercase dark:bg-neutral-900">
            <tr>
          ...
          <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
            {type.props.map((prop) => {
              const defaultValue = defaults[prop.name] ?? prop.defaultValue;
              ...
              return (
                <tr className="align-top" key={prop.name}>
                  <td className="px-3 py-2 font-mono text-[13px] whitespace-nowrap text-neutral-900 dark:text-neutral-100">
                    {prop.name}
                    {prop.required && (
                      <span className="text-danger-500" title="Required">
                        *
                      </span>
                    )}
                  </td>
